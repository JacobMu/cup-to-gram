...
- **THEN** a Chrome MV3 artifact SHALL exist at `.output/chrome-mv3/` after step completion

#### Scenario: Firefox artifact produced by following README steps
- **WHEN** a reviewer follows every step in the Build Instructions section on a clean checkout
- **THEN** a Firefox MV2 artifact SHALL exist at `.output/firefox-mv2/` after step completion
...
- **THEN** the script SHALL exit with code 0 and both `.output/chrome-mv3/` and `.output/firefox-mv2/` SHALL exist
